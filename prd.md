...
- 데이터베이스: SQLite
- 프롬프트 생성: OpenAI GPT
- 이미지 생성: Hugging Face Hub — `Qwen/Qwen-Image`,`Anime-style-flux-lora-Large`

## 8. 시스템 아키텍처(개요)
- Django ↔ API 서버(Fast API) ↔ DB
- 동기 작업: 프롬프트 생성 → 이미지 생성 (생성 대기 시간 표시 필요 - 프로그레스/스피너) → 합성(작업 큐)
- 외부 연동
